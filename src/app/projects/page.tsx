...
export default function ProjectsPage() {
  ...
  const visibleProjects = visiblePortfolio.map(({ project }) => project);
  const activeProjects = visibleProjects.filter((project) => project.status === 'active').length;
  const pausedProjects = visibleProjects.filter((project) => project.status === 'on_hold').length;
      ...
      try {
        ...
        window.alert(
          error instanceof Error
            ? error.message
            : 'Suppression impossible pour ce projet.',
        );
      }
    }
  };
